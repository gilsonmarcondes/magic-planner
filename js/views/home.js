...
export function renderHome() {
        ...
        <div class="p-4 max-w-5xl mx-auto animate-fade-in">
            <div class="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
                ...
                <div class="flex gap-2">
                    <button onclick="window.openTripModal()" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition shadow-md font-bold flex items-center">
                        <span class="mr-2 text-xl">+</span> Nova Viagem
                    </button>
                    <label class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300 transition shadow-sm cursor-pointer font-bold text-[10px] uppercase flex items-center">
                        📂 Importar Backup
    ...
    if (appData.trips.length === 0) {
        html += `<div class="col-span-full text-center text-gray-400 py-16 bg-white border border-dashed rounded-xl">Nenhuma viagem encontrada no seu Mapa do Maroto.</div>`;
    } else {
        appData.trips.forEach(t => {
            html += `
                <div class="bg-white border-b-4 border-b-[#d4af37] rounded-xl shadow-sm hover:shadow-xl transition-all p-5 relative group cursor-pointer" onclick="window.goTo('trip', '${t.id}')">
                    ...
                    <p class="text-sm text-gray-500 mt-2 font-medium italic">📅 ${t.days ? t.days.length : 0} dias de aventura</p>
...
// --- GESTÃO DE VIAGENS (EXPORTS PARA O MAIN.JS) ---
...
export async function importData(event) {
    ...
    reader.onload = async (e) => {
        try {
            ...
            if (!parsed.trips) return alert('Arquivo inválido.');
