...
export function renderTrip(container, tripId) {
    ...
    let html = `
        <div class="p-4 max-w-5xl mx-auto pb-24 animate-fade-in">
            ...
            <div class="flex flex-wrap justify-center gap-2 mb-8 border-b border-[#d4af37]/30 pb-4">
                ...
                <button onclick="window.openHotelManager()" class="bg-white border border-[#0c4a6e] text-[#0c4a6e] px-4 py-2 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-50 transition flex items-center gap-2 font-bold uppercase tracking-tighter">🏨 Hotéis</button>
                <button onclick="window.openChecklist()" class="bg-white border border-[#0c4a6e] text-[#0c4a6e] px-4 py-2 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-50 transition flex items-center gap-2 font-bold uppercase tracking-tighter">✅ Checklist</button>
                <button onclick="window.openDocumentsModal()" class="bg-white border border-[#0c4a6e] text-[#0c4a6e] px-4 py-2 rounded-lg text-xs font-bold shadow-sm hover:bg-blue-50 transition flex items-center gap-2 font-bold uppercase tracking-tighter">📄 Documentos</button>
            </div>
    ...
    if (t.days && t.days.length > 0) {
        t.days.forEach((d, index) => {
            ...
            html += `
                <div class="card p-5 cursor-pointer hover:bg-blue-50 transition border-l-4 border-l-[#d4af37] shadow-sm bg-white rounded-xl" onclick="window.goTo('day', '${t.id}', '${d.id}')">
                    ...
                    <p class="text-xs text-slate-500 font-medium line-clamp-1 italic">📍 ${(d.locations || []).map(l => l.name).join(', ') || 'Sem cidades adicionadas'}</p>
        ...
        html += `<div class="col-span-full text-center py-16 text-gray-400 bg-white border border-dashed rounded-xl">Nenhum dia gerado para este roteiro.</div>`;
    }

    html += `
            </div>
            <div class="mt-10 flex justify-center gap-4">
                <button onclick="window.addDay()" class="bg-[#0c4a6e] text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:bg-[#073654] transition">+ Dia Extra</button>
...
export async function deleteDay(dayId) {
    if(!confirm('Apagar este dia permanentemente?')) return;
    const t = appData.trips.find(x => x.id === currentState.tripId);
...
export function addBucketList() { alert("📝 Em breve: Sua lista de desejos!"); }